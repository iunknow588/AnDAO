...
## 1. Multi-Path User Access

Flexible User Onboarding is a foundational capability of AnDao Wallet, designed to cater to diverse user backgrounds and needs through three distinct access paths. It leverages smart accounts to provide a tailored experience for everyone entering the Web3 space.

· 🆕 Gasless Onboarding for Beginners: New users can create a wallet using just an email or social login, with initial gas fees sponsored for a seamless, key-free start.
· 🔄 Smooth Migration for Professionals: Existing EOA users (e.g., MetaMask) can connect their wallet to upgrade to a smart account, maintaining control while gaining advanced features and optional gas sponsorship.
· 🏢 Management Tools for Sponsors: DApps and community operators can act as Paymasters to sponsor gas, set usage policies (limits, whitelists), and manage campaigns for their users.


## 2. Scheduled Voting

Scheduled Voting is a core innovative feature of AnDaoWallet, specifically designed for the two-phase voting mechanism of the commit-reveal model, ensuring fairness and decentralization of the voting process.

- ✅ **Automatic Execution**: The system automatically executes reveal voting without manual intervention

- ⏰ **Time Control**: Supports setting delay time (minutes, hours, days), flexibly arranging reveal timing

- 🎯 **Precise Execution**: Delayed sending after signing, ensuring reveal voting is executed at the optimal moment

## 3. Delayed Voting

- ⏰ **Delayed Voting**: To implement a mechanism that sends signatures when specific conditions are met. Delayed voting primarily serves decentralized blind box packaging systems, ensuring fairness and decentralization of the blind box reveal process